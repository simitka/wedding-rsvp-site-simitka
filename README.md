...
### [🌐 живое демо – потыкать можно здесь →](https://simitka.github.io/wedding-rsvp-site-simitka/)

<sub>демо-витрина на GitHub Pages, без бэкенда: в анкету пускает слово <b>«хинкали»</b>, ответы никуда не улетают</sub>
...
## ✈️ Пасхалка: ААААВИАСЕЙЛС!!

Кнопка музыки в углу умеет больше, чем кажется. Короткий тап – просто вкл/выкл звука. А вот если **зажать её и не отпускать**, начинается «обратный отсчёт непонятно чего»: кот-комментатор сперва вежливо отговаривает («там будет ТАКОЕ… может, всё-таки не надо?»), потом звонит Ульяне и умывает лапы, а откуда-то из-за кадра уже орут «ААААВИАСЕЙЛС!!».

<table>
<tr>
<td align="center"><img src="docs/screenshots/egg-countdown.jpeg" width="420" alt="Отсчёт: кот ещё отговаривает" /></td>
<td align="center"><img src="docs/screenshots/egg-aviasales.jpeg" width="420" alt="ААААВИАСЕЙЛС!! и голова-иллюминатор" /></td>
</tr>
<tr>
<td align="center"><sub><b>10…8</b> · кот ещё верит, что ты передумаешь</sub></td>
<td align="center"><sub><b>5</b> · «…что? кто это сказал??» – голова-иллюминатор уже в кадре</sub></td>
</tr>
<tr>
<td align="center"><img src="docs/screenshots/egg-panic.jpeg" width="420" alt="Паника на финальных 3-2-1" /></td>
<td align="center"><img src="docs/screenshots/egg-stamp.jpeg" width="420" alt="Штамп «авиарежим: вкл» и фейерверк" /></td>
</tr>
<tr>
<td align="center"><sub><b>3-2-1</b> · «я умываю лапы!!» – вибрация, вспышки, драма</sub></td>
<td align="center"><sub><b>0</b> · «он додержал!!!» – штамп и фейерверк</sub></td>
</tr>
</table>

Отпустишь палец до нуля – всё отменится, и кот выдохнет («фух. пронесло.»). Додержишь – фейерверк, штамп **«АВИАРЕЖИМ: ВКЛ»** и наоборотный авиарежим: обычная музыка сменяется секретным треком, а над страницей начинают летать синие самолётики с котятами. Теперь всё летает и поёт.

<div align="center">
<img src="docs/screenshots/egg-planes.jpeg" width="640" alt="Самолётики с котятами летают над страницей" />

<sub>тык по кнопке глушит всё как раньше, второй тык возвращает обычную музыку</sub>
</div>
...
Тот же сайт на английском живёт на **https://wedding.simitka.io/en** (анкета — `/en/rsvp`). Версии независимы: ссылок друг на друга нет, состояние в `localStorage` у каждой своё; общие только `/api/` и `/assets/`.

`index.en.html` — **генерируемый** файл: правки вносятся в русский `index.html`, а английский пересобирается.

```bash
python3 scripts/i18n/extract.py   # собрать русские строки → scripts/i18n/strings.json
python3 scripts/i18n/build_en.py  # index.html + translations.json → index.en.html
```

Если в русской версии появился новый текст, `build_en.py` падает со списком непереведённых строк: дописываешь их в `scripts/i18n/translations.json` (ключ — русский текст) и повторяешь сборку. Скрипт же проверяет, что в результате не осталось кириллицы, и сам чинит структурные отличия английской версии: абсолютные пути к ассетам, роутинг `/en/rsvp`, отдельные ключи `localStorage`, гостевое слово `khinkali`.
...
├── index.en.html         # английская версия (/en, /en/rsvp) — генерируется из index.html
├── scripts/i18n/         # генератор английской версии (extract.py, build_en.py, translations.json)
