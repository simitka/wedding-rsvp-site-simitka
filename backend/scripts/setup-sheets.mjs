...
await batch([
  ...
  // фото-колонки K/L шире (~120px) и строки данных повыше (~90px), чтобы миниатюры
  // =IMAGE было видно; строку заголовка (индекс 0) не трогаем
  { updateDimensionProperties: {
    range: { sheetId: answersId, dimension: 'COLUMNS', startIndex: 10, endIndex: 12 },
...
// (B/C, 1 или 2). Маркер приглашённости — имена, а не секретное слово:
// слово может быть ещё не вписано, а гость уже в списке. «Приедут»-счётчики
// берут людей из ответа D («Да, оба» = 2) — там пара могла ответить за одного.
const A = "'" + ANSWERS + "'!";
const rowGuests = `((${A}B2:B1000<>"")+(${A}C2:C1000<>""))`;
...
const svodka = [
  ...
  ['Заполнили анкету (человек)', `=${guestsWhere(`${A}I2:I1000=TRUE`)}`],
  ['Ещё не ответили (человек)', `=${guestsWhere(`${A}D2:D1000=""`)}`],
  ['Приедут (человек)', `=${peopleWhere('1')}`],
  ['Ответили «приедем» (человек)', `=${guestsWhere(`LEFT(${A}D2:D1000,2)="Да"`)}`],
  ['Ответили «не приедем» (человек)', `=${guestsWhere(`${A}D2:D1000="Нет"`)}`],
  ['Нужен трансфер (человек)', `=${peopleWhere(`${A}E2:E1000="Нужен"`)}`],
  ['Едут сами (человек)', `=${peopleWhere(`${A}E2:E1000="Не нужен"`)}`],
  ['Свободных мест у попутчиков', `=SUM(${A}F2:F1000)`],
  ['Останутся ночевать (человек)', `=${peopleWhere(`${A}G2:G1000="Остаюсь"`)}`],
  ['Оплатят домик сами (человек)', `=${peopleWhere(`${A}H2:H1000="Заплачу"`)}`],
  ['Домик за счёт молодожёнов (человек)', `=${peopleWhere(`${A}H2:H1000="Не заплачу"`)}`],
];
